...
def factors(num)
  (1..num).map{|el| el if num % el == 0}.compact
end
...
class Array
    ...
    if block_given?
      ...
      begin
        ...
        self.each_with_index do |el, idx|
          ...
          if yield(el, self[idx+1]) == -1
            self[idx], self[idx+1] = self[idx+1], self[idx]
            swap = true
          end
        end
      end until swap == false
      self
    else
      self.bubble_sort_function
    end
  end
  ...
  def bubble_sort_function
    return self if self.length < 2
    begin
      swap = false
      self.each_with_index do |el, idx|
        if compare_two_nums(el, self[idx+1]) == 1
          self[idx], self[idx + 1] = self[idx + 1], self[idx]
          swap = true
        end
      end
...
def substrings(string)
  working_str = string.dup
  counter = 1
  final_arr = []
  while counter <= string.length
    final_arr << subs(working_str)
    working_str = working_str.split("")
    working_str.shift(1)
    working_str =  working_str.join("")
    counter += 1
  end
...
def subs(string)
  working_str = string.dup.split("")
  final_arr = []
  counter = 1

  while counter <= string.length
    final_arr << working_str.shift(counter).join
    working_str = string.dup.split("")
    counter += 1
  end
...
def subwords(word, dictionary)
  ...
  dictionary.each do |dict|
     final_arr << substrings(word).select{|el| el if el == dict}
  end
...
class Array
  def my_each(&prc)
    ...
    while counter < arr_dup.length
      arr_dup[counter] = prc.call(arr_dup[counter])
      counter += 1
    end
