...
def missing_numbers(nums)
  return (nums.first..nums.last).to_a - nums
end
...
def base2to10(binary)
  return binary.chars.reverse.each_with_index.reduce(0){|acc, (el, idx)| acc += el.to_i*(2**idx)}
end
...
class Hash
  ...
  def my_select(&prc)
    hsh = {}
    self.each {|k, v| hsh[k] = v if prc.call(k,v)}
    hsh
  end
...
class Hash
  ...
  def my_merge(hash, &prc)
    hsh = {}
    if !block_given?
      prc = Proc.new {|k, oldv, newv| hsh[k] = newv}
    end
    ...
    self.each do |k, oldv|
      ...
      hsh[k] = prc.call(k, oldv, newv)
    end

    hash.each do |k, newv|
      hsh[k] = hash[k] if hsh.keys.none?{|k1| k == k1}
    end
...
def longest_palindrome(string)
  counter = 3
  final_num = 0
  while !string.empty? && (counter < string.length)
    string.chars.each_cons(counter) do |el|
      if el == el.reverse && el.length > final_num
        final_num = el.length
      end
    end
    counter += 1
  end
  final_num > 0 ? final_num : false
end
